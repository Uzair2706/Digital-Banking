...
class CustomerServicesTest {
    ...
    void creatingCustomerForApplication() {
        ...
        AgeResponseDto ageResponseDto = new AgeResponseDto("Uzair", "43", "3456");
    ...
    @Test
    void updateCustomer_withValidInput_shouldUpdateTheCustomer() {
    ...
    @Test
    void updateCustomers_withInvalidRegex_shouldThrowAnException() {
        String userName = "UzairKhan2706";
        String code = "CUS-UPDATE-NFD-001";
        Customer customer = new Customer();
        PatchCustomerRequest patchCustomerRequest = new PatchCustomerRequest();
        patchCustomerRequest.firstName("Uzair").lastName("Khan").phoneNumber("72268030");
        Mockito.when(validationHelper.validateCustomer(userName, code)).thenReturn(customer);
        Mockito.when(regexValues.getPhoneRegex()).thenReturn("^[0-9]{10}$");
        Assertions.assertThrows(BadRequestExceptions.class, () -> customerServices.updateCustomer(userName, patchCustomerRequest));
    }
